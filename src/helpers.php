...
if (!function_exists('call_class_func')) {
     ...
     * @return mixed
     * @throws \Oyta\Plugin\Exceptions\PluginException
     * @throws \Oyta\Plugin\Exceptions\PluginInvalidException
     */
    function call_class_func($class, $method, $args = [])
    {
        if (is_string($class)) {
            ...
            if (!class_exists($class)) {
                throw new \Oyta\Plugin\Exceptions\PluginInvalidException("Plugin [$className] not found.");
            }
        } else {
        ...
        if (!method_exists($class, $method)) {
            throw new \Oyta\Plugin\Exceptions\PluginInvalidException("Plugin [$className] has not method: [$method].");
        }

        try {
            return call_user_func_array([$class, $method], $args);
        } catch (\Exception $e) {
            throw new Oyta\Plugin\Exceptions\PluginException($e->getMessage(), $e->getCode(), $e->getPrevious());
        }
    }
}
...
if (!function_exists('add')) {
     ...
     */
    function add($name, $callback, $args = 0, $priority = 10, $mode = \Oyta\Plugin\Hook::ACTION_MODE)
    {
        if (empty($name)) {
            return false;
        }
        if (empty(\Oyta\Plugin\Manager::$hooks[$name])) {
            $config = [
                'mode' => $mode
            ];
            \Oyta\Plugin\Manager::$hooks[$name] = new \Oyta\Plugin\Hook($name, $config);
        }
        return \Oyta\Plugin\Manager::$hooks[$name]->add($callback, $priority, $args);
    }
}
...
if (!function_exists('add_action')) {
    ...
    {
        return add($name, $callback, $args, $priority, \Oyta\Plugin\Hook::ACTION_MODE);
    }
}
...
if (!function_exists('add_filter')) {
    ...
    {
        return add($name, $callback, $args, $priority, \Oyta\Plugin\Hook::FILTER_MODE);
    }
}
...
if (!function_exists('add_once')) {
    ...
    {
        return add($name, $callback, $args, $priority, \Oyta\Plugin\Hook::ONCE_MODE);
    }
}
...
if (!function_exists('remove')) {
    ...
    {
        if (empty($name) || empty(\Oyta\Plugin\Manager::$hooks[$name])) {
            return true;
        }
        return \Oyta\Plugin\Manager::$hooks[$name]->remove($callback);
    }
}
...
if (!function_exists('remove_all')) {
    ...
    function remove_all($name, $priority = null)
    {
        ...
        return \Oyta\Plugin\Manager::$hooks[$name]->removeAll($priority);
    }
}
...
if (!function_exists('have')) {
    ...
    {
        if (empty($name) || empty($callback) || empty(\Oyta\Plugin\Manager::$hooks[$name])) {
            return false;
        }
        return \Oyta\Plugin\Manager::$hooks[$name]->have($callback);
    }
}
...
if (!function_exists('play')) {
    ...
    function play($name, ...$args)
    {
        ...
        return \Oyta\Plugin\Manager::$hooks[$name]->play($args);
    }
}
